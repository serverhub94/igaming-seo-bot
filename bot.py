...
# Ограничить доступ только своим user_id (пусто = доступ для всех)
...
WRITE:
  ...
  — Expert but readable
  — Commercial but trustworthy
  — Informative but not bloated

DO NOT write:
  — Generic AI filler ("In the world of online gambling...")
  — Repetitive intros or empty SEO paragraphs
  — Robotic phrasing or guaranteed wins claims
  — Unnatural keyword insertions
...
Before outputting verify:
  ...
  — Each H2 has 200-350+ words
  — Every H2 starts with 1-2 framing sentences
  — Every H2 has table/bullet/numbered list
  — Visual formats alternate across sections
  — All lists have 4+ items, tables have 3+ rows
...
User sends short task descriptions like:
...
Apply the full system above and generate the complete article.
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "Просто напиши параметры — получи полный SEO-текст.\n\n"
        "*Формат:*\n"
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await status_msg.edit_text(
            f"❌ Ошибка: {str(e)}\n\n"
            "Проверь DEEPSEEK_KEY в Railway Variables.\n"
...
def main():
    ...
    print("🤖 Бот на DeepSeek запускается...")
