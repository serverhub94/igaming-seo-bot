...
CONTENT_PROFILES = {
    ...
    "text_review_v2": {
        ...
        "h2_default": None,   # из SERP медианы
        "faq_default": None,  # из SERP медианы
        "instructions": """
...
""",
    ...
    "mono_v3": {
        ...
        "h2_default": None,   # None = брать из SERP медианы конкурентов
        "faq_default": None,  # None = брать из SERP медианы конкурентов
        "instructions": """
...
═══ SEPARATOR (only if multiple versions requested) ═══
If and ONLY IF instructed to generate multiple versions:
...
If generating ONE version — output single article only, no separators.
"""
...
"""
    ...
    # params значения приоритетнее профиля; профиль приоритетнее хардкод-дефолта
    words = params.get("words") or profile.get("words_default") or "3000-3500"
    h2    = params.get("h2")    or profile.get("h2_default")    or 8
    faq   = params.get("faq")   or profile.get("faq_default")   or 7
...
Target words:  {words} words minimum (do NOT stop early — write the full volume)
H2 sections:   {h2 if h2 else "determine from SERP median of competitor pages"}
FAQ questions: {faq if faq else "determine from SERP median of competitor pages"}
NOTE: If H2/FAQ are "determine from SERP" — analyze competitor pages first,
calculate median H2 count and FAQ count, then match or slightly exceed median.
...
IMPORTANT — NUMBER OF TEXTS TO GENERATE: {qty}
{"Generate exactly ONE complete article." if qty == 1 else f"Generate exactly {qty} unique versions separated by === VERSION N ===."}
Language: {params.get("lang", "English")}
Follow all system rules: section intro → visual element → body text.
Output directly — no meta-commentary. No preamble.
"""
...
async def cmd_types(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "→ 1500-2000 слов, H2/FAQ из SERP медианы\n\n"
        "*text_review + V3*\n"
        "→ Listicle Top-N, обзоры казино\n"
        "→ 2500-3000 слов, H2/FAQ из SERP медианы\n\n"
        "*text_review + V4*\n"
        "→ Экспертные ревью, флагманы\n"
        "→ 3500-4500 слов, H2/FAQ из SERP медианы\n\n"
        "━━━━━━━━━━━━━━━━━━━━━━━━\n"
        "*mono + V2* (text_mono)\n"
        ...
        "→ 3000-3500 слов, H2/FAQ из SERP медианы\n\n"
        "━━━━━━━━━━━━━━━━━━━━━━━━\n"
        "*Как указать в ТЗ:*\n"
        "`Тип генерации: V4`\n"
        "`Тип: text_mono`\n\n"
...
def parse_tz_format(text: str):
    ...
    params = {
        ...
        "words": None,  # None = профиль подставит дефолт
        "h2": None,     # None = профиль подставит дефолт
        "faq": None,    # None = профиль подставит дефолт
        "qty": 1, "keywords": [], "urls": [],
    ...
    for line in lines:
        ...
        if line.startswith("http"):
            ...
            # НЕ берём бренд из URL — он возьмётся из ключей позже
            continue
    ...
    # Бренд: приоритет 1) явный тег "Бренд:", 2) первый ключ, 3) URL, 4) дефолт
    if not params["brand"]:
        if params["keywords"]:
            # Берём первый ключ и очищаем от лишних слов
            first_kw = params["keywords"][0]
            # Если ключ составной (ggbet casino) — берём первое слово как бренд
            brand_word = first_kw.split()[0].title()
            params["brand"] = brand_word
        elif params["urls"]:
...
async def handle_quick(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if tz_params:
        ...
        await update.message.reply_text(
            ...
            f"🔢 H2: `{'из SERP анализа' if not tz_profile['h2_default'] else tz_profile['h2_default']}` · FAQ: `{'из SERP анализа' if not tz_profile['faq_default'] else tz_profile['faq_default']}`\n\n"
    ...
    params = {
        ...
        "words":     parts[4] if len(parts) > 4 else None,
        "qty":       int(parts[5]) if len(parts) > 5 and parts[5].isdigit() else 1,
        "h2": None, "faq": None, "keywords": [], "urls": [],
    }
